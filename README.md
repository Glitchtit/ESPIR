...
The master **and both slave builds** support Zigbee OTA firmware updates via Zigbee2MQTT.

**One-time migration:** the OTA-capable firmware uses a new dual-slot partition table,
so the first time you load it you must flash that device over USB (`cd <app> && idf.py flash`,
where `<app>` is `master`, `slave`, or `slave-pcb`). The device re-pairs to your Zigbee
network once after this flash. All later updates are wireless. (Slaves are sleepy end
devices, so their wireless transfers are slower — do them on good battery/USB power.)
...
Restart Z2M. Each ESPIR device then appears in the OTA tab. Click "Check", then "Update"
